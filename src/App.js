...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "UPDATE_EXPENSE": {
      const expensesDuplicate = [...state.expenses]; // Use spread operator to avoid mutating the original state
...
function App() {
  ...
  const updateExpense = async (expense) => {
    ...
    const expenseRef = doc(db, "expenses", expense.id); // Get a reference to the expense document
    await updateDoc(expenseRef, expense); // Update the expense in Firestore
